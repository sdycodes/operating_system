...
{ 
	...
	struct Page *ppage;
	Pte *ppte;
    if(srcva>=UTOP||dstva>=UTOP) return -1;
	ppage = NULL;
    ...
    if(ppage==0)   return -1;
    if(!(*ppte&(0xFFF)&PTE_R)&&(PTE_R&perm))    return -1;
    //ppage = pa2page(PTE_ADDR(ppte[PTX(round_srcva)]));
...
{
    // Your code here
    if(va>0xffffffff||dev+0xa0000000>0xffffffff)
        return -1;
    bcopy(va, dev + 0xa0000000, offset);
    return 0;
}
...
{
    // Your code here
    if(dev + 0xa0000000>0xffffffff||va>0xffffffff)
        return -1;
    bcopy(dev + 0xa0000000, va, offset);
    return 0;
}
